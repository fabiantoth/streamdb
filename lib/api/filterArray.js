...
const compareLogic = require('../db/helpers/compare-logic')
...
const validateOper = (oper) => {
    ...
    if (!options.includes(oper)) {
        throw new CustomError('VALIDATION_ERROR', `Invalid operator found in arr 2nd position: ${oper}`)
    }
}
...
const filterFunction = (arr, chunkedFilter) => {
    ...
    return arr.filter(item => {
        if (field === '$item') {
            ...
            if (compareLogic[oper](dotProp.get(item, field), searchVal)) {
                return item
            }
        }
    })
