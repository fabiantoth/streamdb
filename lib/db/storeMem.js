...
class StoreMem extends EventEmitter {
    ...
    constructor() {
        ...
        this.on('createRequest', (requestId, data) => {
            this.createRequest(requestId, data)
        })

        this.on('addTask', (requestId, taskId, data) => {
            this.addTask(requestId, taskId, data)
        })

        this.on('completeTask', (requestId, taskId, result) => {
            this.completeTask(requestId, taskId, result)
        })
    }
...
StoreMem.prototype.createRequest = function (requestId, data) {
...
StoreMem.prototype.completeRequest = function (requestId) {
    ...
    this.emit(requestMarker, this._queue[requestId].completed)
}

StoreMem.prototype.closeRequest = function (requestId) {
    if (!this._queue[requestId]) {
        throw new CustomError('CACHE_ERROR', `Request id '${key}' does not exists`)
    }

    delete this._queue[requestId]
}

StoreMem.prototype.addTask = function (requestId, taskId, task) {
    this._tasks.push(task)

    this._queue[requestId].tasks.push(task)
    this._queue[requestId].taskCount++
}

StoreMem.prototype.completeTask = function (requestId, taskId, taskResult) {
    this._queue[requestId].completedTasks++
    this._queue[requestId].completed[taskId] = taskResult

    let result = this._tasks.filter(task => task.taskId === taskId)
    if (result) {
        result[0].data = taskResult 
        

        // if this is the last task complete request
        if (this._queue[requestId].completedTasks === this._queue[requestId].taskCount) {
...
StoreMem.prototype.getPendingTasks = function (requestId) {
    if (this._queue[requestId]) {
        return this._queue[requestId].taskCount
    }
    return null
}
