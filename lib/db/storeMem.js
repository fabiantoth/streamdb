...
class StoreMem extends EventEmitter {
    ...
    constructor() {
        ...
        this.on('addVirtualModel', (key, id) => {
            this.addVirtualModel(key, id)
        })
...
StoreMem.prototype.addVirtualModel = function (key, model) {
    if (!this._virtuals[key]) {
        this._virtuals[key] = model
    }
}

StoreMem.prototype.getVirtualModel = function (key, model) {
    if (this._virtuals[key]) {
...
// run scheudled tasks in sequence
    ...
    if (this._queue[requestId].scheduledTasks.size) {
        ...
        for (let [key, task] of tasks) {
            try {
                ...
                // throw new CustomError('CACHE_ERROR', {
                //     requestId,
                //     taskId: task.taskId,
                //     methodCall: task.methodCall,
                //     message:  `Error trying to run scheduled task`,
                //     error: e
                // })
            }
        }
