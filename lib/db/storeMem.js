...
class StoreMem extends EventEmitter {
    ...
    constructor() {
        ...
        this.on('addTask', (requestId, task) => {
            this.addTask(requestId, task)
        })
...
StoreMem.prototype.completeRequest = function (requestId) {
    ...
    this.emit(requestMarker, this._queue[requestId].completedMap)
}
...
StoreMem.prototype.addTask = function (requestId, task) {
    if (this._queue[requestId].completedMap.has(task.mapId)) {
        throw new CustomError('CACHE_ERROR', `Task key '${task.mapId}' already exists`)
    }

    this._queue[requestId].tasks.push(task)
    this._queue[requestId].taskCount++
    this._queue[requestId].completedMap.set(
        task.mapId,
        task.data
    )
}

StoreMem.prototype.completeTask = function (requestId, mapId, taskResult) {
    if (this._queue[requestId].completedMap.has(mapId)) {
