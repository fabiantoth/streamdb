...
module.exports = class DB {
    ...
    getDbMeta() {
        if (!this.dbName || typeof this.dbName !== 'string') {
            throw new Error('Db name is required and must be a valid string')
        }
        this.dbMeta = getMetaFile(`${this.dbPath}/${this.dbName}.meta.json`)
        this.idType = this.dbMeta.defaultModel.id
    ...
    getColMeta(colName) {
        if (this.dbMeta) {
            if (!this.dbMeta.collections.includes(colName)) {
                throw new Error(`Collection "${colName}" does not exist`)
            }
    ...
    addCollection(colName, options) { // create new collection
        return new Promise (async (resolve, reject) => {
            // camel case name
            const name = camelcase(colName)

            if (name.length > 24 || name.length < 2) {
                reject(`Collection name must be between 2-24 chars, attempted string is ${name.length} chars long`)
                return
            }

            let exists = this.dbMeta.collections.includes(name)
            if (exists) {
            ...
            try {
                let metaFile = await createCollection(name, this.dbMeta, options)
                if (!metaFile) {
    ...
    collection(colName) {
        if (!colName || typeof colName !== 'string') {
            throw new Error('Collection name is required and must be a valid string')
        }
        ...
        return new Collection(colName, this.dbMeta)
    }
}