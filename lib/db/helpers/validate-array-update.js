...
const validateArrayUpdate = (pathExpr, arrValues, includeParam, schema, dbName) => {
    let validatedArr = []
    let isRel = false
    ...
    if (expr.length > 1) {
        if (!['=', '!=', '===', '!=='].includes(expr[1])) {
            throw new CustomError('VALIDATION_ERROR', `Value, '${expr[1]}', is not a valid pathExpr operator for updateArray()`)
        }
    }
    ...
    // if not a schema type and strict is false return array values w/no validation
    if (!schemaType && !settings.strict) {
        return { validatedArr: arrValues }
    }

    const isEmpty = schemaType.embeddedType ? schemaType.embeddedType.isEmpty : schemaType.isEmpty
    ...
    // empty arrays/nested-arrays
    } else {
        // for cases of empty nested array embeds, allow using an object matcher
        if (matcher !== '$item' && expr[1] !== '===') {
            throw new CustomError('VALIDATION_ERROR', `Matchers that dont use the '$item' keyword can only be used with (===) operator`)
        }
        validatedArr = schemaType.validate(arrValues)
    }
