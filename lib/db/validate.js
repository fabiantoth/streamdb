...
module.exports = {
    ...
    defaultModel: (model) => {
        ...
        if (model === undefined) {
            ...
            if (type !== '[object Object]') {
                throw new Error(`[Type Error]: expected "defaultModel" to be an object, received instead: ${type}`)
            }
            ...
            if (model.maxValue !== undefined) {
                if (typeof model.maxValue !== 'number' || model.maxValue < 6) {
                    throw new Error(`[Validation Error] "defaultModel.maxValue" must be a number type, with value greater than 6`)
                }
                if (validationModel.id === '$uid' && model.maxValue > 36) {
                    throw new Error(`[Validation Error] "defaultModel.maxValue" for '$uid' type must be must between 6-36. Received: ${model.maxValue}`)
                }
