...
module.exports = {
    ...
    docSizeOk: (storeMax, obj) => { // validate document size is not greater than storeMax
        ...
        if (size > storeMax) {
            throw new CustomError('VALIDATION_ERROR', `Document size (${size}) exceeds store max (${storeMax}). Increase store max value or reduce document size`)
        }
    },
    ...
    validateDirName: (string) => {
        ...
        if (!string.match(regex)) {
            throw new CustomError('VALIDATION_ERROR', `DB name can only include alphanumeric characters and hyphenated words`)
        }
    
        if (string.length < 2 || string.length > 26) {
            throw new CustomError('VALIDATION_ERROR', `DB name must be between 2-26 characters`)
        }
    ...
    defaultModel: (model) => {
        ...
        if (model === undefined) {
            validationModel = { // default model settings
                type: 'schema',
                id: '$incr',
                maxValue: 10000          
            }
            ...
            if (type !== '[object Object]') {
                throw new CustomError('TYPING_ERROR', `Expected "defaultModel" to be an object, received instead: ${type}`)
            }
            model.type !== undefined ? (model.type === 'default' ? validationModel.type = 'default' : validationModel.type = 'schema') : validationModel.type = 'schema'
            model.id !== undefined ? (model.id === '$uid' ? validationModel.id = '$uid' : validationModel.id = '$incr') : validationModel.id = '$incr'
            
            if (model.maxValue !== undefined) {
                if (typeof model.maxValue !== 'number' || model.maxValue < 6) {
                    throw new CustomError('VALIDATION_ERROR', `"defaultModel.maxValue" must be a number type, with value greater than 6`)
                }
                if (validationModel.id === '$uid' && model.maxValue > 36) {
                    throw new CustomError('VALIDATION_ERROR', `"defaultModel.maxValue" for '$uid' type must be must between 6-36. Received: ${model.maxValue}`)
                }

                validationModel.maxValue = model.maxValue
            } else {
                // $incr/idMaxCount (default=10000), $uid/uidLength (default=11)
                validationModel.id === '$uid' ? validationModel.maxValue = 11 : validationModel.maxValue = 10000
            }
        }
