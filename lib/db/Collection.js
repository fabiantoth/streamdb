...
module.exports = class Collection {
    ...
    useModel(modelName) {
        ...
        const key = `${this.colName.dbName}/${modelName}`
        storeMem.emit('addVirtualModel', key, this.model)
    ...
    getById(id) { // get one document by id
        return new Promise (async (resolve, reject) => {
            try {
                ...
                const dbName = this.colMeta.dbName 
                const key = `${dbName}/${this.colName}`
        ...
        return new Promise (async (resolve, reject) => {
            try {
                if (!Array.isArray(documents)) {
                    reject(new Response('error', `InsertMany argument must be an array`))
                }
    ...
    updateOne(updateObj) { // update a single document
        return new Promise(async (resolve, reject) => {
            try {
                if (!updateObj.id) {
                    reject(new Response('error', `UpdateOne argument must include an object with document id`))
                }
                ...
                if (!colIds.includes(updateObj.id)) {
                    reject(new Response('error', `Document with id "${updateObj.id}" does not exist`))
                }

                let res = await this.Queue.add(updateOneDocument, this.colMeta, updateObj, this.model)
