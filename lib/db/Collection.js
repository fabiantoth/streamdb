...
const createRouter = require('./collection/create-router')
const createModel = require('./collection/create-model')
...
module.exports = class Collection {
    ...
    useModel(modelName) {
        // TODO: add name validation, cleanup vModel workflow
     ...
     * 
     * @returns {Promise} Promise object with response message
     */
     generateRouter() {
        return new Promise (async (resolve, reject) => {
            try {
                let data = await createRouter(this.colName, this.dbMeta)
                let response = new Response('success', `Router file for "${this.colName}" created successfully!`)
    ...
    generateModel(modelName) {
        return new Promise (async (resolve, reject) => {
            try {
                let settings = this.colMeta.model

                // disallow creating model if it already exists
                if (settings.name) {
                    reject(new Response('error', `Model already exists for "${this.colName}" collection`))
                }
                if (settings.type !== 'schema') {
                    reject(new Response('error', `Model files can only be created for 'schema' type validation`))
                }

                if (modelName) {
                    settings.name = modelName || null
                }
                
                modelName = await createModel(settings, this.colName, this.dbMeta)
                let response = new Response('success', `Model file "${modelName}" created successfully!`)
    ...
    getDocs(idsArr) {
        return new Promise (async (resolve, reject) => {
            try {
                ...
                let documents = await getManyDocs(colInfo, idsArr)
        ...
        return new Promise (async (resolve, reject) => {
            try {
                if (this.filters.length === 0) {
                    throw new CustomError('VALIDATION_ERROR', `setProperty() must be preceded by where() method`)
                }
    ...
    deleteProperty(propertyPath) {
        return new Promise (async (resolve, reject) => {
            try {
                if (this.filters.length === 0) {
                    throw new CustomError('VALIDATION_ERROR', `deleteProperty() must be preceded by where() method`)
                }
                if (typeof propertyPath !== 'string') {
    ...
    insertInto(propertyPath, arrValues) {
        return new Promise (async (resolve, reject) => {
            try {
                if (this.filters.length === 0) {
                    throw new CustomError('VALIDATION_ERROR', `insertInto() must be preceded by where() method`)
                }
                if (typeof propertyPath !== 'string') {
    ...
    removeFrom(propertyPath, arrValues) {
        return new Promise (async (resolve, reject) => {
            try {
                if (this.filters.length === 0) {
                    throw new CustomError('VALIDATION_ERROR', `removeFrom() must be preceded by where() method`)
                }
                if (typeof propertyPath !== 'string') {
    ...
    updateArray(pathExpr, updatesArr) {
        return new Promise (async (resolve, reject) => {
            try {
                if (this.filters.length === 0) {
                    throw new CustomError('VALIDATION_ERROR', `updateArray() must be preceded by where() method`)
                }
    ...
    and(exp) {
        if (this.filters.length === 0) {
            throw new CustomError('VALIDATION_ERROR', `and() must be preceded by where() method`)
        }
        if (typeof exp !== 'string') {
    ...
    or(exp) {
        if (this.filters.length === 0) {
            throw new CustomError('VALIDATION_ERROR', `or() must be preceded by where() method`)
        }
        if (typeof exp !== 'string') {
    ...
    limit(num) {
        if (this.filters.length === 0) {
            throw new CustomError('VALIDATION_ERROR', `limit() must be preceded by where() method`)
        }
    ...
    sort(sortBy, sortOrder) {
        if (this.filters.length === 0) {
            throw new CustomError('VALIDATION_ERROR', `sort() must be preceded by where() method`)
        }
    ...
    offset(num) {
        if (this.filters.length === 0) {
            throw new CustomError('VALIDATION_ERROR', `offset() must be preceded by where() method`)
        }
        if (typeof num !== 'number') {
    ...
    include(fieldsArray) {
        if (this.filters.length === 0) {
            throw new CustomError('VALIDATION_ERROR', `include() must be preceded by where() method`)
        }
    ...
    exclude(fieldsArray) {
        if (this.filters.length === 0) {
            throw new CustomError('VALIDATION_ERROR', `exclude() must be preceded by where() method`)
        }
        if (!Array.isArray(fieldsArray)) {
    ...
    populate(fieldsArray) {
        if (this.filters.length === 0) {
            throw new CustomError('VALIDATION_ERROR', `populate() must be preceded by where() method`)
        }
        if (this.colMeta.model.type !== 'schema') {
    ...
    find() {
        return new Promise (async (resolve, reject) => {
            try {
                if (this.filters.length === 0) {
                    throw new CustomError('VALIDATION_ERROR', `find() must be preceded by where() method`)
                }
                this.verifyColExists()
                let refAssets
