...
module.exports = class Collection {
     ...
     * - expressions
     *      (update many)               -> ('title = "Article 1'), ('content.length = 300'),
     *      (update first matching)     -> ('detail.age != $undefined'),
     * - Dates 
     *      (update many)               -> ('updated_at $before/$after "2021-04-21T06:08:24.105Z") 
     *      (update first matching)     -> ('updated_at $on "2021-04-21")
     * 
     * pathExpr cases for primitives & dates:
     * - single paths not permitted
     * - expressions:
     *      (update many)              -> ('$item = "John"'), ('$item = 100')
     *      (update first matching)    -> ('$item === "John"'), ('$item != "John"')
     * - Dates
     *      (update many)              -> ('$item $before/$after "2021-04-21T06:08:24.105Z")
     *      (update first matching)    -> ('$item $on "2021-04-21")
     */
    updateArray(pathExpr, updatesArr) {
        return new Promise (async (resolve, reject) => {
            try {
                ...
                updatesArr = validateArrayUpdate(pathExpr, updatesArr, includeParam, this.model.schema)
                const data = await Queue.add(updatePropertyArray, this.colMeta, this.filters, includeParam, pathExpr, updatesArr, timestamps)
