...
module.exports = class Collection {
    ...
    setProperty(propertyPath, value) { // runs query chain (filters only) and sets property
        return new Promise (async (resolve, reject) => {
            try {
                ...
                value = validateSetProperty(propertyPath, value, schema)
                this.verifyColExists()
    ...
    include(arr) {
        if (!Array.isArray(arr)) {
            throw new CustomError('TYPING_ERROR', `include() argument must be an array, received: ${typeof arr}`)
        }
        if (arr.length === 0) {
    ...
    exclude(arr) {
        if (!Array.isArray(arr)) {
            throw new CustomError('TYPING_ERROR', `exclude() argument must be an array, received: ${typeof arr}`)
        }
        if (arr.length === 0) {
    ...
    populate(arr) {
        ...
        if (!Array.isArray(arr)) {
            throw new CustomError('TYPING_ERROR', `populate() argument must be an array, received: ${typeof arr}`)
        }
        if (arr.length === 0) {
        ...
        const populateArr = validatePopulate(arr, schema)

        this.parameters.push({ type: 'populate', properties: populateArr })
