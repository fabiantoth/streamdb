    ...
    insertInto(propertyPath, arrValues) { // runs query chain (filters only) and adds values to array
        return new Promise (async (resolve, reject) => {
            try {
                ...
                if (this.model) {
                    let { validatedArr, isRel } = validateInsertToValues(propertyPath, arrValues, this.model)

                    arrValues = validatedArr
                    docRel = isRel // pass for duplicate filtering purposes
    ...
    updateArray(propertyPath, updateFn) {
        return new Promise (async (resolve, reject) => {
            try {
                ...
                const data = await Queue.add(updatePropertyArray, this.colMeta, this.filters, propertyPath, updateFn, this.model)
                let response = new Response('success', `Query ran successfully`, data)
