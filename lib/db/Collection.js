...
module.exports = class Collection {
    ...
    verifyModelExists() {
        if (!this.model) {
            throw new Error(`[validationError]: model is required for collection "${this.colName}" schema validation`)
        }
    }
    ...
    insertOne(document) { // add one document to collection
        return new Promise (async (resolve, reject) => {
            try {
                this.verifyColExists()
                let response = await insertOneDocument(this.colMeta, document, this.model)
                resolve(response)
    ...
    insertMany(documents) { // add one or more documents to collection
        return new Promise (async (resolve, reject) => {
            try {
                this.verifyColExists()
                let response = await insertManyDocuments(this.colMeta, documents, this.model)
                resolve(response)
            } catch (e) {
                reject(e)
            }
    ...
    updateOne(update) { // update a single document
        return new Promise(async (resolve, reject) => {
            try {
                this.verifyColExists()
                let res = await updateOneDocument(this.colMeta, update, this.model)
    ...
    updateMany(updates) { // update one or more documents
        return new Promise(async (resolve, reject) => {
            try {
                this.verifyColExists()
                let res = await updateManyDocuments(this.colMeta, updates, this.model)
    ...
    deleteOne(id) { // delete a single document from collection by id
        return new Promise (async (resolve, reject) => {
            try {
                this.verifyColExists()
                let deletedId = await deleteOneDocument(this.colMeta, id)
    ...
    deleteMany(idsArray) { // delete one or more documents from collection
        return new Promise (async (resolve, reject) => {
            try {
                this.verifyColExists()
                let deletedIds = await deleteManyDocuments(this.colMeta, idsArray)
    ...
    find() { // runs query chain
        return new Promise (async (resolve, reject) => {
            try {
                this.verifyColExists()
                let data = await queryCollection(this.colPath, this.filters, this.parameters)
        ...
        return new Promise (async (resolve, reject) => {
            try {
                if (typeof propertyPath !== 'string') {
                    reject(`[typeError]: setProperty() propertyPath argument must be a string "path.to.property"`)
                }

                if (value === undefined) {
                    reject(`[validationError]: setProperty() value argument cannot be undefined`)
                }
                ...
                const data = await setProperty(this.colMeta, this.filters, propertyPath, value, timestamps)
    ...
    deleteProperty(propertyPath) { // runs query chain (filters only) and deletes property
        return new Promise (async (resolve, reject) => {
            try {
                if (typeof propertyPath !== 'string') {
                    reject(`[typeError]: deleteProperty() method requires a string "path.to.property"`)
                }
                this.verifyColExists()
                let timestamps = null 
        ...
        return new Promise (async (resolve, reject) => {
            try {
                ...
                if (!Array.isArray(arrValues)) {
                    reject(`[typeError]: insertTo() arrValues must be an array`)
                }

                if (arrValues.length === 0) {
                    reject(`[validationError]: insertTo() arrValues cannot be empty`)
                }
                ...
                const data = await insertIntoArray(this.colMeta, this.filters, propertyPath, arrValues, timestamps)
    ...
    removeFrom(propertyPath, arrValues) {
        return new Promise (async (resolve, reject) => {
            try {
                ...
                const data = await removeFromArray(this.colMeta, this.filters, propertyPath, arrValues, timestamps)
    ...
    updateArray(propertyPath, updateFn) {
        return new Promise (async (resolve, reject) => {
            try {
                if (typeof propertyPath !== 'string') {
                    reject(`[typeError]: updateArray() propertyPath must be a string "path.to.property"`)
                }

                if (updateFn === undefined || typeof updateFn !== 'function') {
                    reject(`[typeError]: updateArray() second argument requires a callback function, received: ${typeof updateFn}`)
                }

                if (this.parameters.length > 0) {
                    reject(`[validationError]: cannot use any parameters (sort, limit, include, etc.) with updateArray()`)
                }

                this.verifyColExists()

                const data = await updatePropertyArray(this.colMeta, this.filters, propertyPath, updateFn, this.model)
    ...
    where(exp, filterFn) { // starts query chain
        if (typeof exp !== 'string') {
            throw new Error(`[typeError]: first where() argument must be a string, received: ${typeof exp}`)
        }

        if (filterFn) {
            if (typeof filterFn !== 'function') {
                throw new Error('[typeError]: second where() argument must be a function')
            }
            
            this.filters.push(exp)
            this.filters.push('array')
    ...
    and(exp) {
        if (typeof exp !== 'string') {
            throw new Error(`[typeError]: and() argument must be a string, received: ${typeof exp}`)
        }

        if (this.filters.length < 1) {
            throw new Error(`[validationError]: and() methods cannot be used before opening where()`)
        }
    ...
    or(exp) {
        if (typeof exp !== 'string') {
            throw new Error(`[typeError]: or() argument must be a string, received: ${typeof exp}`)
        }

        if (this.filters.length < 1) {
            throw new Error(`[validationError]: or() methods cannot be used before opening where()`)
        }
    ...
    limit(num) {
        if (typeof num !== 'number') {
            throw new Error(`[typeError]: limit() argument must be a number, received: ${typeof num}`)
        }
    ...
    sort(sortBy, sortOrder) {
        if (typeof sortBy !== 'string') {
            throw new Error(`[typeError]: sort(), sortBy argument must be a string, received: ${typeof sortBy}`)
        }

        if (sortOrder !== undefined) {
            if (sortOrder !== 'asc' && sortOrder !== 'desc') {
                throw new Error(`[validationError]: sort(), sortOrder argument may only be 'asc' or 'desc', received: ${sortOrder}`)
            }
        }
    ...
    offset(num) {
        if (typeof num !== 'number') {
            throw new Error(`[typeError]: offset() argument must be a number, received: ${typeof num}`)
        }
    ...
    include(arr) {
        if (!Array.isArray(arr)) {
            throw new Error(`[typeError]: include() argument must be an array, received: ${typeof num}`)
        }
        if (arr.length === 0) {
            throw new Error(`[validationError]: include() array must have at least one string value`)
        }
        arr.forEach(item => {
            if (typeof item !== 'string') {
                throw new Error(`[typeError]: include() array can only contain string values`)
            }
        })
        this.parameters.filter(param => {
            if (param.type === 'exclude') {
                throw new Error(`[validationError]: include() cannot be used with exclude()`)
            }
            if (param.type === 'include') {
                throw new Error(`[validationError]: include() params may only be set once`)
            }
        })
    ...
    exclude(arr) {
        if (!Array.isArray(arr)) {
            throw new Error(`[typeError]: exclude() argument must be an array, received: ${typeof num}`)
        }
        if (arr.length === 0) {
            throw new Error(`[typeError]: exclude() array must have at least one string value`)
        }
        arr.forEach(item => {
            if (typeof item !== 'string') {
                throw new Error(`[typeError]: exclude() array can only contain string values`)
            }
        })
        this.parameters.filter(param => {
            if (param.type === 'include') {
                throw new Error(`[validationError]: exclude() cannot be used with include()`)
            }
            if (param.type === 'exclude') {
                throw new Error(`[validationError]: exclude() params may only be set once`)
            }
        })
    ...
    geoSearch(params) {
        return new Promise (async (resolve, reject) => {
            try {
                this.verifyColExists()
                let data = await queryCollection(this.colPath, this.filters, this.parameters, params)
