...
Schema.prototype.fieldHandler = function (field, params) {
    ...
    if (Object.prototype.toString.call(params) === '[object Array]' && typeof params !== 'function') {
        ...
        let schemaType = new typeAssign(field, params, this.settings.strict)
...
Schema.prototype.castNestedObjects = function (field, params) {
    ...
    const nestedObject = new typeAssign(field, params, this.settings.strict)
...
Schema.prototype.validatePath = function (obj, path) {
    ...
    if (fields.length === 1) {
        ...
        let result = schemaType.validate(obj[`${path}`])
        if (result !== undefined) {
            validObj[`${path}`] = result
            return validObj
        } else {
            return undefined
        }
        ...
        fields.forEach((field, i) => {
            if (!schemaType) {
                ...
                if (i === fields.length - 1) {
                    let result = schemaType.validate(obj[`${field}`])
                    result !== undefined 
                        ? validObj[`${field}`] = result
                        : validObj = undefined
                }
