...
Schema.prototype.castNestedObjects = function (field, params) {
    ...
    // update any nested object subdocs
    if (nestedObject) {
        if (nestedObject.nestedSubDocs.length) {
            nestedObject.nestedSubDocs.forEach(nested => {
                this.subDocs.push({
                    dbName: nested.dbName,
                    collection: nested.collection,
                    localField: field,
                    nestedField: nested.field,
                    
                    model: nested.model,
                    path: `${nested.dbName}/${nested.collection}#${field}.${nested.field}`
                })
            })
        }
    }
...
Schema.prototype.castDocumentModels = function (params, field) {
    ...
    let path = `${DocumentModel.dbName}/${DocumentModel.colName}#${field}`
