                ...
                updates.forEach(updateObj => {
                    ...
                    if (Object.keys(graph).length) {
                        for (let path in graph) {
                            dotProp.has(updateObj, path) ? needsDocs.push(updateObj.id) : null
                        }
                    }
...
const defaultUpdates = async (colMeta, updates) =>{
    ...
    updates.forEach(update => {
        ...
        if (!currentDocs[index] || !update.id) {
            throw new Error(`Could not match document to update document with id "${update.id}"`)
        }
