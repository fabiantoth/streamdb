    ...
    return new Promise(async (resolve, reject) => {
        try {
            updates.forEach(document => {
                let colIds = storeMem.getCollectionIds(`${colMeta.dbName}/${colMeta.colName}`, null)
                if (!colIds.includes(document.id)) {
                    throw new CustomError('VALIDATION_ERROR', `Document with id "${document.id}" does not exist`)
                }
            })
