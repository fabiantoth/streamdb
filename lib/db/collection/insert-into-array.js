...
const setArrayProperty = (propertyPath, arrValues, obj, timestamps, docRel) => {
    ...
    // remove any duplicate values, or objects with id values for refs or doc objects
    if (docRel) {
        const noDuplicates = arrValues.filter((key, i) => {
            let found = arrValues.filter(item => {
                if (item.id) {
                    if (item.id === key.id) {
                        return item
                    }
        ...
        if (currArr.length) {
            const filteredResults = noDuplicates.filter(item => {
            ...
            noDuplicates.length ? updatedArr = noDuplicates : null
        }
