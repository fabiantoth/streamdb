...
const model = (modelName, schema, colMeta) => {
    try {
        ...
        if (!storeMem.getCollection(this.colMeta, null)) {
            storeMem.emit('init', this.colMeta, schema._TypedSchema, schema.settings)
        }

        if (!storeMem.getSchema(key)) {
            storeMem.emit('setSchema', key, schema._TypedSchema, schema.settings)
        }
        ...
        if(schema._TypedSchema) {
            ...
            throw new CustomError('DB_ERROR', `Failed to get _TypedSchema for "${modelName}" model`)
        }
