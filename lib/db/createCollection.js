...
const createCollection = async (colName, dbMeta, settings) => {
    try {
        let model = {}
        let storeMax = dbMeta.storesMax

        if (settings) {
            settings.storeMax ? storeMax = validate.validateStoresMax(settings.storeMax) : null
...
const validateModelObject = (dbMeta, colName, modelOptions) => {
    ...
    if (model.type === 'schema') {
        // if initSchemas set to false, set name & path to null
        if (!dbMeta.initSchemas) {
            model.name =  modelOptions.name !== undefined ? createModelName(validate.isString(modelOptions.name)) : null 
            model.path = model.name ?  `${modelsPath}/${model.name}.js` : null
