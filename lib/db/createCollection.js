...
//          name: String                               Select model name, default will be generated capitalized singular colName ('users' => 'User' model name)
...
const createCollection = async (colName, dbMeta, settings) => {
    try {
        ...
        if (settings.model.type === 'schema' && dbMeta.initSchemas) {
            dbMeta.models.push(`${model.name}.js`)
...
const validateModelObject = (dbMeta, colName, modelOptions) => {
    ...
    if (dbMeta.models.includes(`${model.name}.js`)) {
        throw new Error(`[Validation Error]: Model name "${model.name}" already exists.`)
    }
