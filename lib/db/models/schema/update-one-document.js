    ...
    return new Promise (async (resolve, reject) => {
        try {
            ...
            if (docUpdate.id === undefined) {
                throw new Error(`[validationError]: update objects must include an id, received: ${docUpdate.id}`)
            }
                ...
                if (schemaKeys.includes(field)) {
                    ...
                    if (field === 'id') {
                        ...
                        if (docUpdate.id === null) {
                            throw new Error(`[validationError]: id field values cannot be null`)
                        }
                        ...
                        if (schemaField.instance) {
                            ...
                            if (schemaField instanceof SchemaType || schemaField instanceof Any) {
                                ...
                                if (schemaField.instance === 'array') {
                                    ...
                                    if (schemaField.params.length > 0) {
...
const updateSchemaDocument = (docObj, sdModel) => {
    return new Promise (async (resolve, reject) => {
        try {
            ...
            if (!document) {
                throw new Error('[schemaError]: Could not update embedded schema document')
            }
...
const updateManySchemaDocuments = (sdModel, docObjArray) => {
    return new Promise (async (resolve, reject) => {
        try {
            ...
            if (!documents) {
                throw new Error('[schemaError]: Could not create array of schema documents')
            }
