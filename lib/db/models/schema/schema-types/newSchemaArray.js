...
class newSchemaArray extends SchemaType {
    ...
    constructor(field, params) {
        ...
        if (Array.isArray(params) || Array.isArray(params.type)) {
    ...
    setEmbedOptions() {
        ...
        if (Object.prototype.toString.call(arrEmbed) === '[object Array]') {
            typeAssign = assignType('nestedObject')
            schemaType = new typeAssign(this.field, this.params, Array)
            this.params = [schemaType]

        } else if (typeof arrEmbed === 'function') {
            typeAssign = assignType(arrEmbed)
            schemaType = new typeAssign(this.field, this.params.type, Array)

        } else {
            // Embedded Schemas translated into schemaObjects 
            typeAssign = assignType('nestedObject')
            schemaType = new typeAssign(this.field, this.params, Array)
        }
