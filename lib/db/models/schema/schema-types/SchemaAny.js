    ...
    setTypeOptions() {
        ...
        for (let param in this.params) {
            ...
            if (!this.options.includes(param)) {
                throw new Error(`[Schema Error]: "${param}" is not a valid SchemaAny option for field "${this.field}"`)
            }
            ...
            if (param === 'default') {
                if (typeof ruleValue !== 'string' || typeof ruleValue !== 'number' || ruleValue !== null) {
                    throw new Error(`[Schema Error]: 'default' values for type Any can only be null, string, or number types`)
                }
            }

            if (param === 'anyOf') {
                if (!Array.isArray(ruleValue)) {
                    throw new Error(`[Schema Error]: 'anyOf' option must be an array, received: "${typeof ruleValue}"`)
                }
                if (ruleValue.length < 1) {
                    throw new Error(`[Schema Error]: 'anyOf' option must have at least one value`)
                }
                
                ruleValue.forEach(item => {
                    if (typeof item !== 'function') {
                        if (typeof item !== 'string' && typeof item !== 'number') {
                            throw new Error(`[Schema Error]: 'anyOf' non-type values can only be strings and numbers`)
                        }
                    } else if (!anyOfOptions.includes(item)) {
                        throw new Error(`[Schema Error]: 'anyOf' values can only be [String|Number|Boolean|Date] types, and/or strings & number values`)
                    }
                })
            }
