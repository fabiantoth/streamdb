...
class SchemaString extends SchemaType {
    ...
    setTypeOptions() {
        ...
        for (let param in this.params) {
            ...
            if (param === 'required' || param === 'unique' || param === 'lowercase' || param === 'capitalize') {
                if (typeof ruleValue !== 'boolean') {
                    throw new Error(`[schemaError]: '${param}' field can only be set to true or false`)
                }
            }

            if (param === 'lowercase' && param === 'capitalize') {
        ...
        if (value === undefined) {
            ...
            if (this.rules) {
                ...
                if (this.rules.capitalize) {
                    ...
                    let dashedWords = value.split('-')
                    value = words.map(word => word.trim().charAt(0).toUpperCase() + word.trim().slice(1)).join(' ')
                    value = dashedWords.map(word => word.trim().charAt(0).toUpperCase() + word.trim().slice(1)).join('-')
                }
                // apply trim (should be part of validation?)
                if (this.rules.trim) {
                    value = value.trim()
                }
