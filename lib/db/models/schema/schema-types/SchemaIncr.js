...
class Schema$incr extends $incr {
    ...
    validate(id, colMeta, exists) {
        if (typeof id !== 'number') {
            throw new Error(`[Schema Error]: $incr can only be a number, received: ${typeof id}`)
        }
    ...
    checkIdExists(id, colMeta) {
        ...
        if (!exists) {
            throw new Error(`[Schema Error]: document with id "${id}" does not exist in collection '${colMeta.colName}'`)
        }
    ...
    checkIdNotExists(id, colMeta) {
        ...
        let exists = allIds.includes(id)
        if (exists) {
            throw new Error(`[Schema Error]: document with id "${id}" already exists in collection '${colMeta.colName}'`)
        }
