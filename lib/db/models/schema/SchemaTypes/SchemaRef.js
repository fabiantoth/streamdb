...
SchemaRef.prototype.validate = function (refObj) {
    ...
    const idExists = storeMem.findIdExists(this.params.collection, refObj.$ref)

    if (!idExists) {
        throw new CustomError('VALIDATION_ERROR', `Document with id '${refObj.$ref}' does not exist in '${this.params.collection}' collection`)
    }
