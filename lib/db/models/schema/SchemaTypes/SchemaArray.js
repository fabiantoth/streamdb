...
SchemaArray.prototype.setEmbedOptions = function () {
    ...
    // Nested array --> [[],[String],..]
        ...
        // only accept [[String,Number]]
        if (embedded.length === 1) {
            const allowedEmbeds = [String,Number]
...
SchemaArray.prototype.validate = function (values, update, dbName) {
    ...
    if (!this.isEmpty) {
        ...
        values = this.validateEmbedOptions(values, update, dbName)
...
SchemaArray.prototype.validateEmbedOptions = function (values, update, dbName) {
    if (this.embeddedType.isEmpty) {
        ...
        values.forEach(val => this.embeddedType.validate(val))
        return values
    }
    ...
    if (this.embeddedType.instance === 'nestedObject') {
        ...
        values.forEach(value => {
            ...
            let exists = this.embeddedType.idExists(id, dbName) 
            if (!exists) {
