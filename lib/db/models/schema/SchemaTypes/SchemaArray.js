...
SchemaArray.prototype.setTypeOptions = function () {
    ...
    for (let param in this.params) {
        ...
        if (param === 'default') {
            if (ruleValue !== null) {
                throw new CustomError('SCHEMA_ERROR', `"${param}" values for arrays can only accept null or [], received: ${ruleValue}`)
            }
        }
        ...
        if (param === 'validate') {
            if (typeof ruleValue !== 'function') {
                throw new CustomError('SCHEMA_ERROR', `'validate' option must be an function, received: ${typeof ruleValue}`)
            }
        }
...
const getType = (func) => {
    if (func === String) {
        ...
        return 'date'
    } else if (func === Array || Object.prototype.toString.call(func) === '[object Array]') {
        return Array
    } else if (func === $incr) {
        return '$incr'
    } else if (func === $uid) {
        ...
        throw new Error(`Could not find type for "${func}"`)
    }
}
