...
const handleTypeKeyword = (field, params, colMeta) => {
    ...
    if (params.type === $incr) {
        ...
        console.log(field, params.type, typeof params.type)
        // assign schema type
...
const handleNestedFields = (params) => {
    ...
    if ('type' in params && Object.prototype.toString.call(params.type) === '[object Object]') {
        if ('$ref' in params.type) {
            throw new Error(`[schemaError]: a $ref object cannot be assigned to "type" keyword field`)
        }
    }

    // const nestedObj = castTypes(params, SchemaClass) || {}
