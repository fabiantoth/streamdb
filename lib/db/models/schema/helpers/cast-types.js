    ...
    try {
        ...
        if(colMeta) {
            if (!('id' in schema)) {
                ...
                if (colMeta.model.id === '$incr') {
                    idResult = handleFunction('id', $incr)
                } else {
                    idResult = handleFunction('id', $uid)
                }
        ...
        for (const field in schema) {
            ...
            if (typeof params === 'function') {
                let fnResult = handleFunction(field, params)
...
const handleFunction = (field, params) => {
    if (field === 'type') {
        throw new CustomError('SCHEMA_ERROR', `the "type" keyword is reserved`)
    }

    const typeAssign = assignType(params)
    const schemaType = new typeAssign(field, params)
    return schemaType
}

const handleTypeKeyword = (field, params) => {
    const typeAssign = assignType(params.type)
    const schemaType = new typeAssign(field, params)
    return schemaType
}
