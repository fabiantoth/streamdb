const { SchemaType } = require('./SchemaTypes/index')
...
const addOneDocument = (docObj, model) => {
    return new Promise (async (resolve, reject) => {
        try {
            ...
            const _TypedSchema = model._TypedSchema
            const settings = model.settings
            const schemaKeys = Object.keys(_TypedSchema)

            let document = {}
            
            for (const field in _TypedSchema) {
                ...
                if (schemaField === null) {
                ...
                } else if (schemaField.instance === 'document') {
                    // handle nested Document
                    const sdModel = model.schema[field]
                    let result = await createSchemaDocument(objectField, sdModel).catch(e => { throw new CustomError('SCHEMA_ERROR', e.message) })
                    
                    document[field] = result

                } else if (schemaField.instance === '$ref') {
                    // handle a nested $ref object
                    // check if object data 
                    if (Object.prototype.toString.call(objectField) === '[object Object]') {
                        if (!('$ref' in objectField)) {
                            // attempt to get model using schema model collection ref
                            const refModel = await schemaField.getModel(schemaField.params.collection, model.colMeta.colPath).catch(e => { throw new CustomError('SCHEMA_ERROR', e.message) })

                            // if id in object field
                            if (objectField.id) {
                                validate.idTypeMatch(refModel.colMeta.model.id, objectField.id)
                                
                                // create $ref object to set with new id/collection
                                document[field] = {
                                    collection: schemaField.params.collection,
                                    $ref: objectField.id
                                }
                            } else {
                                // validate object data with createSchemaDocument & create new doc
                                let refResult = await createSchemaDocument(objectField, refModel).catch(e => { throw new CustomError('SCHEMA_ERROR', e.message) })
    
                                // return $ref object to set with new id/collection
                                document[field] = {
                                    collection: schemaField.params.collection,
                                    $ref: refResult.id
                                }
                        ...
                        if (schemaField.params.length > 0 && typeof schemaField.params !== 'function') {
                            if (schemaField.params[0].instance === 'schema') {
                                ...
                                if (objectField === undefined) {
                                    ...
                                    let results = await createManySchemaDocuments(sdModel, objectField).catch(e => { throw new CustomError('SCHEMA_ERROR', e.message) })
                                    if (!results) {
                                        throw new CustomError('SCHEMA_ERROR', `Could not create array of schema document`)
                                    }
                        ...
                        for (let key in schemaField) {
                            if ('params' in schemaField[key]) {
                                if (schemaField[key].params.required) {
                                    throw new CustomError('VALIDATION_ERROR', `'${key}' field nested in ${field}.${key} is required`)
                                }
                            }
                        }
                    }
                    ...
                    if (fieldType !== '[object Object]' && objectField !== undefined) {
                        throw new CustomError('TYPING_ERROR', `Expected ${model.modelName} ${field} to be an object, received: ${fieldType}`)
                    }
...
const traverseNested = async (schemaField, objectField, nestedSchema, settings) => {
    ...
    for (const field in schemaField) {
        ...
        if (subSchemaField.instance === 'document') {
            // handle nested embedded Document
...
const createSchemaDocument = (docObj, sdModel) => {
    return new Promise (async (resolve, reject) => {
        try {
            ...
            if (!document) {
                throw new CustomError('SCHEMA_ERROR', `Could not create embedded schema document`)
            }
...
const createManySchemaDocuments = (sdModel, docObjArray) => {
    return new Promise (async (resolve, reject) => {
        try {
            ...
            documents = await doc.save(documents)

            if (!documents) {
                throw new CustomError('SCHEMA_ERROR', `Could not create embedded schema documents`)
            }
