...
const traverse = (pathInfo, docObj, _TypedSchema, requestId, strict, parentInfo) => {
    ...
    for (let field in _TypedSchema) {
        ...
        if (parentField) {
            docPathInfo.path = `${parentField}.${field}`
        } else {
            docPathInfo.path = field
        }
        ...
        if (field === 'id' && !parentField) {
            document.id = docPathInfo.parentId
            ...
            if (schemaField.instance === 'document') {
                if (objectField) {
                    ...
                    const taskId = addDocEmbed(docPathInfo, objectField, schemaField, requestId)
                    if (taskId) {
            ...
            // handle nested $ref Obj document
                ...
                if (objectField) {
                    ...
                    const taskId_or_ref = addRefEmbed(docPathInfo, objectField, schemaField, requestId)
            ...
            // TODO: set replace this with ending 'Any' for deeper nested objects?
                ...
                if (objectField) {
                    result = traverse(docPathInfo, objectField, schemaField._TypedSchema, requestId, strict, parentInfo)
                ...
                if (objectField === null) {
                    ...
                    if (objectField) {
                        ...
                        if (schemaField.isDocEmbed) {
                            taskId_or_ref = addArrayDocEmbeds(docPathInfo, objectField, schemaField.embeddedType, requestId)

                        // [$ref] Embed
                        } else {
                            taskId_or_ref = addArrayRefsEmbeds(docPathInfo, objectField, schemaField.embeddedType, requestId)
                        }
