...
const addOneDoc = (docObj, model, requestId) => {
    ...
    for (let field in _TypedSchema) {
        ...
        if (field !== 'id') {
            ...
            // check for parent ref, if collection matches generate $ref obj to add to field
            const symbols = Object.getOwnPropertySymbols(docObj)
            if (symbols.length) {
                symbols.forEach(symbol => {
                    if (symbol.description === 'parentId') {
                        if (docObj[symbol].collection === schemaField.params.collection) {
                            let ref = {
                                collection: docObj[symbol].collection,
                                $ref: docObj[symbol].id
                            }
                            parentRef = ref
                        }
                    }
            ...
            // handle nested objects
                ...
                if (objectField) {
                    result = addNestedObject(pathInfo, objectField, schemaField, requestId, settings.strict, parentRef)
...
const addNestedObject = (pathInfo, nestedObject, objectInstance, requestId, strict, parentRef) => {
    ...
    for (let nestedField in _TypedSchema) {
        ...
        // handle nested $ref Obj document
            ...
            if (objectField) {
                ...
                if (parentRef) {
                    validNestedObj[nestedField] = parentRef
                }
            }
        ...
        // TODO: set replace this with ending 'Any' object?
            ...
            if (objectField) {
                result = addNestedObject(nestedField, objectField, schemaField, requestId, strict, parentRef)
...
const addDocEmbed = (pathInfo, objectField, docInstance, requestId, ref) => {
    // create parent id/col meta to pass down to sub document
