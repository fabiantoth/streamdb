...
const addRefEmbed = (pathInfo, objectField, refInstance, requestId) => {
    ...
    if (typeof objectField === 'string' || typeof objectField === 'number') {
        refId = objectField

    } else if (objectField.id) {
        refId = objectField.id
    }

    // if id is provided, validate & return result
    if (refId) {
        validRef = refInstance.idExists(refId)
        if (!validRef) {
            throw new CustomError('VALIDATION_ERROR', `Document with id '${refId}' does not exist in '${refInstance.params.collection}' collection`)
        }
