                ...
                if (objectField === null) {
                    ...
                    if (objectField) {
                        ...
                        if (schemaField.isDocEmbed) {
                            taskId_or_ref = validateArrayEmbeds(pathInfo, objectField, schemaField.embeddedType, requestId)
        ...
        // Arrays embedded documents or document $ref objects
            ...
            if (objectField === null) {
                ...
                if (objectField) {
                    ...
                    // [Document] Embed 
                    if (schemaField.isDocEmbed) {
                        taskId_or_ref = validateArrayEmbeds(nestedPathInfo, objectField, schemaField.embeddedType, requestId)
...
const validateArrayEmbeds = (pathInfo, objectsArray, docInstance) => {
    // create parentInfo data to pass down to sub document
    docInstance['parentInfo'] = {
        dbName: pathInfo.dbName,
        collection: pathInfo.collection,
...
const addArrayRefsEmbeds = (pathInfo, objectsArray, refInstance, requestId) => {
    ...
    const docInstance = refInstance.getDocInstance(pathInfo.path)
    const validObjects = validateArrayEmbeds(pathInfo, objectsArray, docInstance)
