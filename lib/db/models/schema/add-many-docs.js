            ...
            if (field !== 'id') {
                ...
                if (schemaField.instance === 'document') {
                    if (objectField) {
                        ...
                        // build batch
                        result = batchTask(pathInfo, objectField, schemaField, id, requestId, batch)
                        if (result) {
                ...
                // handle document $ref embed
                    ...
                    if (objectField) {
                        ...
                        if ('$ref' in objectField) {
                            ...
                            // build batch
                            result = batchTask(pathInfo, objectField, docInstance, id, requestId, batch, ref)
                            if (result) {
                                batch[pathInfo.path] = result
                ...
                // handle nested Objects                    
                    ...
                    if (objectField) {
                        const { validNestedObj, nestedBatch } = addNestedObject(pathInfo, objectField, schemaField, requestId, id, dbName, settings.strict, parentInfo)
                    ...
                    if (objectField === null) {
                        ...
                        if (objectField) {
                            ...
                            if (schemaField.isDocEmbed) {
                                taskId = addArrayDocEmbeds(pathInfo, objectField, schemaField.embeddedType, requestId)
...
const addNestedObject = (pathInfo, nestedObject, objectInstance, requestId, id, dbName, strict, parentInfo) => {
    ...
    for (let nestedField in _TypedSchema) {
        ...
        if (schemaField.instance === 'document') {         
            if (objectField) {
                // build nestedBatch
                result = batchTask(nestedPathInfo, objectField, schemaField, id, requestId, nestedBatch)
                if (result) {
        ...
        // handle nested $ref Obj document
            ...
            if (objectField) {
                ...
                if ('$ref' in objectField) {
                    ...
                    result = batchTask(nestedPathInfo, objectField, docInstance, id, requestId, nestedBatch, ref)
                    if (result) {
                        nestedBatch[nestedPath] = result
        ...
        // Arrays embedded documents or document $ref objects
            ...
            if (objectField === null) {
                ...
                if (objectField) {
                    ...
                    // [Document] Embed 
                    if (schemaField.isDocEmbed) {
                        taskId = addArrayDocEmbeds(nestedPathInfo, objectField, schemaField.embeddedType, requestId)

                    // [$ref] Embed
                    } else {
                        taskId = addArrayRefsEmbeds(nestedPath, objectField, schemaField.embeddedType, dbName, requestId)
                    }
...
const batchTask = (pathInfo, objectField, docInstance, id, requestId, batch, ref) => {
