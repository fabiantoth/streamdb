...
const addManyDocuments = (docObjects, model, requestId) => {
    ...
    if (Object.prototype.toString.call(docObjects) !== '[object Array]') {
        throw new CustomError('VALIDATION_ERROR', 'addMany argument must be an array')
    }
    if (docObjects.length === 0) {
        throw new CustomError('VALIDATION_ERROR', 'addMany array must contain at least one object')
    }
...
const scheduleTask = (path, validData, docInstance, requestId, methodCall) => {
    ...
    // console.log('scheduleTask -> ', methodCall, validData)
