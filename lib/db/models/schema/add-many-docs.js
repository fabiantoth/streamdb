        ...
        for (let field in _TypedSchema) {
            ...
            if (field !== 'id') {
                ...
                // handle nested Objects                    
                    ...
                    if (objectField) {
                        const validNestedObj = addNestedObject(pathInfo, objectField, schemaField, requestId, settings.strict, parentInfo)
...
const addNestedObject = (pathInfo, nestedObject, objectInstance, requestId, strict, parentInfo) => {
    ...
    for (let nestedField in _TypedSchema) {
        ...
        if (schemaField.instance === 'document') {         
            if (objectField) {
                // build batch
                batch(nestedPathInfo, objectField, schemaField, requestId)
                // set document id as tempValue identifier
        ...
        // handle nested $ref Obj document
            ...
            if (objectField) {
                ...
                if (validRef) {
                    ...
                    // build batch
                    batch(nestedPathInfo, validDocObj, docInstance, requestId, ref)
                    // set document id as value
