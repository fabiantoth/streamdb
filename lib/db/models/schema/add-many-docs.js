                ...
                if (schemaField.instance === 'document') {
                    if (objectField) {
                        ...
                        if (!batch[path]) {
                            ...
                            batch[path].batch.push(objectField)
                            batch[path].docIds.push(id)
                        }
        ...
        if (schemaField.instance === 'document') {         
            if (objectField) {
                ...
                if (!nestedBatch[path]) {
                    ...
                    nestedBatch[path].batch.push(objectField)
                    nestedBatch[path].docIds.push(id)
                }
                // set document id as tempValue identifier
    ...
    if (batchKeys.length) {
        ...
        for (let key in batch) {
            ...
            if (validDocs) {
                ...
                const taskObjectId = { [key]: docIds }

                const task = {
                    requestId,
                    taskObjectId,
                    docInstance: batch[key].docInstance,
                    methodCall: 'saveMany',
                    values: [
                        validDocs,
                        requestId,
