        ...
        for (let field in _TypedSchema) {
            ...
            if (field !== 'id') {
                ...
                // handle nested $ref Obj document
                    ...
                    if (objectField) {
                        ...
                        if ('$ref' in objectField) {
                            ...
                            if (!idExists) {
                                throw new CustomError('VALIDATION_ERROR', `Document with id '${objectField.$ref}' does not exist in '${schemaField.params.collection}' collection`)
                            }

                            document[field] = validRef
                        } else {
                            // return Document instance
                            const docInstance = getRefInstance(field, dbName, schemaField)
                            if (docInstance) {
                                ...
                                let ref = {
                                    collection: docInstance.model.colName,
                ...
                // handle deeper nested Objects                    
                    ...
                    if (objectField) {
                        result = addNestedObject(field, objectField, schemaField, dbName, settings.strict)
                        if (result) {
    ...
    if (pendingDocs.length > 0) {
        for (const child in docsCarrier) {
            ...
            if (instance.embeddedType) {
                key = `${instance.embeddedType.model.dbName}/${instance.embeddedType.model.colName}`
                instance.embeddedType.saveMany(docs).catch(e => { throw new Error('saveMany() subdocs error: ' + e ) })
            } else {
                key = `${instance.model.dbName}/${instance.model.colName}`
...
const addNestedObject = (field, nestedObject, objectInstance, dbName, strict) => {
    ...
    for (let nestedField in _TypedSchema) {
        ...
        // handle nested $ref Obj document
            ...
            if (objectField) {
                ...
                if ('$ref' in objectField) {
                    ...
                    validNestedObj[nestedField] = validRef
                } else {
                    // return Document instance
                    const docInstance = getRefInstance(nestedField, dbName, schemaField)
                    if (docInstance) {
                        const subDoc = docInstance.addOne(objectField)
        ...
        // handle deeper nested Objects                    
            ...
            if (objectField) {
                result = addNestedObject(nestedField, objectField, schemaField, dbName, strict)
                if (result) {
