                ...
                // handle document $ref embed
                    ...
                    if (objectField) {
                        ...
                        if (validRef) {
                            ...
                            // build batch
                            result = batchTask(pathInfo, validDocObj, docInstance, requestId, batch, ref)
                            if (result) {
                                batch[pathInfo.path] = result
        ...
        // handle nested $ref Obj document
            ...
            if (objectField) {
                ...
                if (validRef) {
                    ...
                    const validDocObj = validateDocEmbed(nestedPathInfo, objectField, docInstance, 'addOne')
                            
                    // build batch
                    result = batchTask(nestedPathInfo, validDocObj, docInstance, requestId, nestedBatch, ref)
                    if (result) {
                        nestedBatch[nestedPath] = result
                        
                        // set document id as value
                        validNestedObj[nestedField] = validDocObj.id
                    }
                }
...
const batchTask = (pathInfo, objectField, docInstance, requestId, batch, ref) => {
    ...
    if (docInstance.instance === 'document' && !ref) {
        docInstance['parentInfo'] = {
            dbName: pathInfo.dbName,
            collection: pathInfo.collection,
            id: pathInfo.parentId
...
const scheduleBatchTasks = (batch) => {
    ...
    if (batchKeys.length) {
        ...
        for (let key in batch) {
            ...
            let validDocs
            if (batch[key].ref) {
                validDocs = batch[key].batch
            } else {
                validDocs = batch[key].docInstance.addMany(batch[key].batch)
            }
