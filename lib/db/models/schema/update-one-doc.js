...
const updateOneDocument = (updateObj, model, requestId, currDoc, isArray) => {
    ...
    let docUpdate = {}
    let pathInfo = { dbName, parentField: null }
...
const traverse = (pathInfo, updateObj, _TypedSchema, requestId, strict, currDoc, isArray) => {
    let dbName = pathInfo.dbName
    let parentField = pathInfo.parentField
    ...
    for (const field in updateObj) {
        if (schemaKeys.includes(field)) {
            if (updateObj[field] === undefined) {
                throw new CustomError('VALIDATION_ERROR', `update values cannot be undefined`)
            }
            ...
            if (field === 'id') {
                ...
                // handle a nested document $ref embed
                    ...
                    if (objectField === null) {
                        ...
                        if (objectField) {
                            let exists = schemaField.idExists(objectField, dbName)
                            if (!exists) {
                ...
                // handle deeper nested Objects
                    ...
                    if (objectField === null) {
                        ...
                        pathInfo.parentField = field
                        result = traverse(pathInfo, objectField, schemaField._TypedSchema, requestId, strict, currDoc, isArray)
