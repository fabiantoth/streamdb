...
const updateOneDocument = (updateObj, model, requestId, currDoc) => {
    ...
    docUpdate = traverse(pathInfo, updateObj, _TypedSchema, requestId, settings.strict, currDoc)
...
const traverse = (parentField, updateObj, _TypedSchema, requestId, strict, currDoc) => {
    // allow passing null
    if (updateObj === null) {
        return null
    }
    ...
    if (currDoc) {
        if (!parentField) {
            ...
            if (dotProp.has(currDoc, parentField)) {
                // console.log('had parent field: ', dotProp.get(currDoc, parentField))
                update = dotProp.get(currDoc, parentField)
                // console.log('parent field -> ', parentField, update)
            }
        }
    }
            ...
            if (field === 'id') {
                ...
                if (schemaField.instance === 'document') {
                    ...
                    if (objectField === null) {
                        ...
                        if (currValue && objectField.id === currValue.id) {
                            const validDoc = addDocEmbedUpdate(path, objectField, schemaField, requestId)
                ...
                // handle deeper nested Objects
                    ...
                    if (objectField === null) {
                        ...
                        result = traverse(field, objectField, schemaField._TypedSchema, requestId, strict, currDoc)
