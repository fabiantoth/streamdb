            ...
            if (field === 'id') {
                ...
                if (schemaField.instance === 'document') {
                    ...
                    if (objectField === null) {
                        ...
                        // validate & schedule task
                        const path = field
                        const taskId = addDocEmbedUpdate(path, objectField, schemaField, requestId)
                        if (taskId) {
                            // set taskId as tempValue identifier
                            docUpdate[field] = taskId
                        }
                    }
                ...
                // handle nested Objects
                    ...
                    if (objectField) {
                        result = updateNestedObject(field, objectField, schemaField, requestId, settings.strict)
                ...
                // Arrays with embedded documents or $ref objects
                    ...
                    // do nothing except set empty array with null/[]
                    if (objectField === null) {
                        docUpdate[field] = []

                    } else {
                        if (Array.isArray(objectField) && !objectField.length) {
                            docUpdate[field] = []
                        }
                    }
...
const updateNestedObject = (field, nestedObject, objectInstance, requestId, strict) => {
        ...
        if (schemaKeys.includes(nestedField)) {
            ...
            if (subSchemaField.instance === 'document') {
                ...
                if (subObjectField === null) {
                    ...
                    const taskId = addDocEmbedUpdate(nestedPath, subObjectField, subSchemaField, requestId)
                    if (taskId) {
                        // set taskId as tempValue identifier
                        nestedUpdate[nestedField] = taskId
                    }
                }
            ...
            // handle a nested document $ref embed
                ...
                if (subObjectField === null) {
                    ...
                    let exists = subSchemaField.idExists(subObjectField)
                    if (!exists) {
                        throw new CustomError('VALIDATION_ERROR', `Document with id '${subObjectField}' does not exist in '${subSchemaField.params.collection}' collection`)
                    }

                    nestedUpdate[nestedField] = subObjectField
                }
            ...
            // handle deeper nested Objects
                ...
                result = updateNestedObject(nestedField, subObjectField, subSchemaField, requestId, strict)
                result !== undefined ? nestedUpdate[nestedField] = result : null

            // Arrays with embedded documents or $ref objects
            } else if (subSchemaField.instance === 'array' && subSchemaField.isDocEmbed || subSchemaField.isRefEmbed) {
                // do nothing except set empty array with null/[]
                if (subObjectField === null) {
                    nestedUpdate[nestedField] = []

                }  else {
                    if (Array.isArray(subObjectField) && !subObjectField.length) {
                        nestedUpdate[nestedField] = []
                    }
                }
...
const addDocEmbedUpdate = (path, updateObj, docInstance, requestId) => {
