            ...
            if (field === 'id') {
                ...
                if (schemaField.instance === 'document') {
                    ...
                    if (objectField === null) {
                        ...
                        const taskObject = addSubDocUpdate(field, objectField, schemaField, colName, requestId)

                        // store call to send at end of validating this doc
                        if (taskObject) {
                            pendingTasks.push({ 
                                docInstance: taskObject.docInstance, 
                                methodCall: 'saveOneUpdate', 
                                values:[taskObject.validUpdateObj, requestId, taskObject.taskId] 
                            })
                        }
                        
                        if (taskObject) {
                            docUpdate[field] = taskObject.taskId
                            // let tempValue = parseInt(requestId.description)
                            // docUpdate[field] = tempValue
                        }
                    }
...
const addSubDocUpdate = (field, updateObj, docInstance, parentCol, requestId) => {
    let timestamp = Date.now()
    let taskId = timestamp
    ...
    if (validUpdateObj) {
        storeMem.emit('addTask', requestId, taskId, taskObject)
