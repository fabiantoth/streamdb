...
//     initSchemas: Boolean,      [default = true]        set to false to not scaffold Schema models for collections
...
//     modelsAutoDelete: Boolean  [default = true]        set to 'false' to not delete model files on collection remove()
//     defaultModel: {
//          type: String,         [default = 'schema']    2 options: ['default', 'schema'] set to 'default' to use no validation
//          id: String,           [default = '$incr']     2 options: String '$uid'[=streamDb.Types.$uid],Number '$incr'[=streamDb.Types.$incr]   
//          maxValue: Number,     [default = 10000]       if ($incr) set upper range limit on max idCount, if ($uid) set id string length (max=36)
//     }
// }
//
...
const createDb = (options = {}) => {
    return new Promise (async (resolve, reject) => {
        ...
        try {
            ...
            // if default validation, set model options to false
            options.defaultModel === 'default' ? options.initSchemas = false : ''
            options.defaultModel === 'default' ? options.modelsAutoDelete = false : ''
