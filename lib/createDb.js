...
const createDb = (options = {}) => {
    return new Promise (async (resolve, reject) => {
        ...
        try {
            ...
            fileSize === undefined 
                ? options.fileSize = 131072 
                : options.fileSize = validate.validateStoresMax(fileSize)
            idType === undefined 
                ? options.idType = '$incr' 
                : options.idType = validate.isIdType(idType)
            idMaxValue === undefined 
                ? options.idMaxValue = options.idType === '$incr' ? 10000 : 11 
                : options.idMaxValue = validate.idMaxValue(options.idType, idMaxValue)
